 ...
 * $Revision: 3 $
 * $Date: 15/12/14 3:53p $
 * @brief    NANO103 series Smartcard (SC) driver header file
 *
 * @note
...
/** @addtogroup NANO103_SC_EXPORTED_CONSTANTS SC Exported Constants
  @{
*/
...
/*@}*/ /* end of group NANO103_SC_EXPORTED_CONSTANTS */
...
#define SC_SET_CLK_PIN(sc, u32OnOff)\
    do {\
        uint32_t reg = (sc)->PINCTL;\
        if(((reg & (SC_PINCTL_PWREN_Msk | SC_PINCTL_PWRINV_Msk)) == 0) ||\
            ((reg & (SC_PINCTL_PWREN_Msk | SC_PINCTL_PWRINV_Msk)) == (SC_PINCTL_PWREN_Msk | SC_PINCTL_PWRINV_Msk)))\
            reg &= ~SC_PINCTL_PWREN_Msk;\
        else\
            reg |= SC_PINCTL_PWREN_Msk;\
            if(u32OnOff)\
